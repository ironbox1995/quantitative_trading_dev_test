...
def train_lstm_regress_model(curve_path, X_train, y_train, l2_reg=L2_REG_COEFFICIENT):
    ...
    # optimizer = torch.optim.Adam(model_setup.parameters(), lr=LR, weight_decay=l2_reg)
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 10 == 0:
            ...
            if train_loss < best_loss:  # if the train loss is better than the current best loss
                ...
                # Save the model_setup as a pt file
                torch.save(model.state_dict(), 'model_setup\{}_predict_model.pt'.format(file_name))
