...
def reformat_historical_kline(start_time='', period='1d'):
    ...
    for code in tqdm(code_list):
        ...
        # 构建存储文件路径
        path = r'{}\data\historical\QMT_stock_data\{}.csv'.format(project_path, code)
...
def reformat_historical_financial_data(start_time=''):
    ...
    for code in tqdm(a_list):  # 仅抽取a股财务数据
        ...
        for table in table_list:
            ...
            # 构建存储文件路径
            path = r'{}\data\historical\QMT_financial_data\{}-{}.csv'.format(project_path, code, table)

            # 保存数据
            fin_df.to_csv(path, index=False, mode='a', encoding='gbk')
...
if __name__ == "__main__":
    download_historical_financial_data()
    reformat_historical_financial_data()
